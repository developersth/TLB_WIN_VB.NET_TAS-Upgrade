...
Public Class frmConfigCommport_main
    Public FormScreenID As Long
    Dim frm_work As Integer = 0 '1=add ,2=Edit

    Private Sub frmConfigCommport_main_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles MyBase.FormClosed
        'UcStatus1.StopThread()
        UcFooter1.StopThread()
        CloseForm(FormScreenID, "")
    End Sub

    Private Sub frmConfigCommport_main_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetScreenResulotion()
        CheckFormResize(Me)
    ...
    Private Sub frmConfigCommport_main_Resize(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Resize
        CheckFormResize(Me)
    End Sub
    ...
    Public Sub Show_Data()
        ...
        sql_str = "SELECT COMP_ID, COMPORT_NO, COMPORT_TYPE, COMPORT_SETTING, DESCRIPTION, IS_ENABLED FROM COMPORT   "
        sql_str = sql_str & "ORDER BY COMP_ID"
        ...
        If Oradb.OpenDys(sql_str, "TableName1", mDataSet) Then
            ...
            Do While dt.Rows.Count > i
                ...
                DataGridView1.Item(0, i).Value = IIf(IsDBNull(dt.Rows(i).Item("COMP_ID")), "", dt.Rows(i).Item("COMP_ID").ToString)
                DataGridView1.Item(1, i).Value = IIf(IsDBNull(dt.Rows(i).Item("COMPORT_NO")), "", dt.Rows(i).Item("COMPORT_NO").ToString)
                DataGridView1.Item(2, i).Value = IIf(IsDBNull(dt.Rows(i).Item("COMPORT_TYPE")), "", dt.Rows(i).Item("COMPORT_TYPE").ToString)
                DataGridView1.Item(3, i).Value = IIf(IsDBNull(dt.Rows(i).Item("COMPORT_SETTING")), "", dt.Rows(i).Item("COMPORT_SETTING").ToString)
    ...
    Private Sub Delete()
        Dim sql_str As String
        Dim mDataSet As New DataSet
        sql_str = " DELETE FROM COMPORT    "
        sql_str = sql_str & " WHERE  COMP_ID = '" + DataGridView1.Item(0, DataGridView1.CurrentRow.Index).Value.ToString + "' "
...
#Region "Menu Event"
    ...
    Private Sub UcMenuAdd_OnClickMnuText(ucName As System.String, ucScreenID As System.Int64) Handles UcMenuAdd.OnClickMnuText
        frmConfigCommport_sub.Close()
        frmConfigCommport_sub.setFrmWork(1)
        frmConfigCommport_sub.ShowDialog()
    End Sub
    ...
    Private Sub UcMenuEdit_OnClickMnuText(ucName As System.String, ucScreenID As System.Int64) Handles UcMenuEdit.OnClickMnuText
        ...
        frmConfigCommport_sub.Close()
        frmConfigCommport_sub.setFrmWork(2)
        frmConfigCommport_sub.setPkId(id)
        frmConfigCommport_sub.ShowDialog()
    End Sub
