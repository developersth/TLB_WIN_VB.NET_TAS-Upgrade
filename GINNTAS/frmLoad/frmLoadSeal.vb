Public Class frmLoadSeal
    Public FormScreenID As Long

    Private Sub frmLoadSeal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetScreenResulotion()
        CheckFormResize(Me)
    ...
    Private Sub frmLoadSeal_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        'UcStatus1.StopThread()
        UcFooter1.StopThread()
        CloseForm(FormScreenID, "")
    End Sub

    Private Sub frmLoadSeal_Resize(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Resize
        CheckFormResize(Me)
    End Sub
    ...
    Private Sub Showdata_GridMain()
        ...
        sql_str = sql_str & " ,O.UPDATE_DATE ,O.UPDATED_BY ,O.LOAD_STATUS,O.SEAL_USE,O.TRIP_NO "
        sql_str = sql_str & " FROM TAS.OIL_LOAD_HEADERS O "
        sql_str = sql_str & " WHERE O.LOAD_STATUS IN (12) AND O.CANCEL_STATUS=0  "
        ...
        If Oradb.OpenDys(sql_str, "TableName1", mDataSet) Then
            ...
            Do While dt.Rows.Count > i
                ...
                DataGridView_Headers.Item(2, i).Value = IIf(IsDBNull(dt.Rows(i).Item("TU_ID")), "", dt.Rows(i).Item("TU_ID").ToString)

                DataGridView_Headers.Item(3, i).Value = IIf(IsDBNull(dt.Rows(i).Item("TU_ID1")), "", dt.Rows(i).Item("TU_ID1").ToString)
                DataGridView_Headers.Item(4, i).Value = IIf(IsDBNull(dt.Rows(i).Item("SEAL_USE")), "", dt.Rows(i).Item("SEAL_USE").ToString)
                DataGridView_Headers.Item(5, i).Value = IIf(IsDBNull(dt.Rows(i).Item("TRIP_NO")), "", dt.Rows(i).Item("TRIP_NO").ToString)

                DataGridView_Headers.Item(6, i).Value = IIf(IsDBNull(dt.Rows(i).Item("DRIVER_NAME")), "", dt.Rows(i).Item("DRIVER_NAME").ToString)
    ...
    Private Sub b_Add_Seal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub UcMenu_Add_Seal_OnClickMnuText(ucName As System.String, ucScreenID As System.Int64) Handles UcMenu_Add_Seal.OnClickMnuText
        If DataGridView_Headers.Rows.Count <= 0 Then Exit Sub
        If DataGridView_Headers.CurrentRow.Index < 0 Then Exit Sub
        frmLoadEditSeal.Show()
    End Sub
