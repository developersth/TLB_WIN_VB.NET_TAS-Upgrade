...
Public Class frmLoadDO_Edit
    ...
    Private Sub frmLoadDO_Edit_Sub_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Clear_frm()
        InitialCombo()
    ...
    End Sub
    Private Function P_UpdateCusDo(ByVal iDo As String) As Boolean
        Dim vCheck As Boolean = False
        Dim sql_str As String
        If Chk_loadNo_do(iDo) Then
            sql_str = _
                            "begin " & _
                            " TAS.UPDATE_CUSTOMER_DO ('" & _
                            iDo & "'); " & _
                            "end;"
            If Oradb.ExeSQL(sql_str) Then
                vCheck = True
        ...
        Return vCheck
    End Function
    Private Function Chk_loadNo_do(iNDo As String) As Boolean
        Dim mDataSet As New DataSet
        Dim dt As DataTable
        Dim i As Integer = 0
        Dim sql_str As String
        Dim vCheck As Boolean = False
        sql_str = _
                      "select  dh.load_no   from  tas.do_header   dh  where  dh.do_no='" & Trim(iNDo) & "' "
        If Oradb.OpenDys(sql_str, "TableName1", mDataSet) Then
            dt = mDataSet.Tables("TableName1")
            Do While dt.Rows.Count > i
                Chk_loadNo_do = IIf((dt.Rows(i).Item("Load_no") = "0"), False, True)
                i = i + 1
            Loop
            vCheck = True
        End If
    ...
    Private Sub Edit()
        ...
        If Oradb.ExeSQL(sql_str) Then
            If P_UpdateCusDo(Trim(txtDoNo.Text)) Then
    ...
    Private Sub bt_DelProduct_Click(sender As Object, e As EventArgs)
    End Sub
