...
Public Class frmMMIProcessSetLevel
        ...
        With MSGridConfig
            For i = 0 To MSGridConfig.RowCount - 1
                If MSGridConfig.Rows(i).Cells(3).Value <> "" Then
                    Call SaveData(Trim(Meter_no), CLng(Trim(MSGridConfig.Rows(i).Cells(0).Value.ToString)), Trim(MSGridConfig.Rows(i).Cells(3).Value.ToString))
                End If
            Next
        End With
        MsgBox("Setting configuration successful ", vbInformation, "Setting")
    End Sub
    Private Sub SaveData(ByVal iMeter_no As String, ByVal iconfig_no As Long, ByVal iValue As String)
        Dim strSQL As String
        strSQL = "begin " & _
                                 " steqi. WRITE_CONFIG_METER_SETTING( '" & _
                                 iMeter_no & _
                                "'," & _
                                 iconfig_no & _
                                ",'" & _
    ...
    Private Sub InitialValue()
        ...
        Dim i As Integer
        strSQL = "select t.* " & _
                       " from steqi.view_config_meter_setting t" & _
                       " Where t.METER_NO='" & Trim(Meter_no) & "'" & _
                       " order by t.METER_NO "
